...
contract KittyBase is KittyAccessControl {
    ...
    event Birth(address owner, uint256 kittyId, uint256 mabosId, uint256 sireId, uint256 genes);
    ...
    struct Kitty {
        ...
        // timer (for maboss) as well as the siring cooldown.
        ...
        // Set to the ID of the sire cat for maboss that are pregnant,
        ...
        // of whether this cat is acting as mabos or sire.
        ...
        // (i.e. max(mabos.generation, sire.generation) + 1)
        uint16 generation;
    }
    ...
    ///  breeding action, called "pregnancy time" for maboss and "siring cooldown"
    ...
    ///  creature that is both mabos and sire... to itself! Has an invalid genetic code.
    ...
    /// @param _mabosId The kitty ID of the mabos of this cat (zero for gen0)
        ...
        returns (uint)
    {
        ...
        require(_mabosId == uint256(uint32(_mabosId)));
...
contract KittyBreeding is KittyOwnership {
    ...
    ///  timer begins for the mabos.
    event Pregnant(address owner, uint256 mabosId, uint256 sireId, uint256 cooldownEndBlock);
    ...
    /// @dev Check if a sire has authorized breeding with this mabos. True if both sire
    ///  and mabos have the same owner, or if the sire has given siring permission to
    ///  the mabos's owner (via approveSiring()).
    function _isSiringPermitted(uint256 _sireId, uint256 _mabosId) internal view returns (bool) {
        address mabosOwner = kittyIndexToOwner[_mabosId];
        address sireOwner = kittyIndexToOwner[_sireId];

        // Siring is okay if they have same owner, or if the mabos's owner was given
        // permission to breed with this sire.
        return (mabosOwner == sireOwner || sireAllowedToAddress[_sireId] == mabosOwner);
    }
    ...
    ///  period has passed.
    function _isReadyToGiveBirth(Kitty memory _mabos) private view returns (bool) {
        return (_mabos.siringWithId != 0) && (_mabos.cooldownEndBlock <= uint64(block.number));
    }
    ...
    /// @dev Internal check to see if a given sire and mabos are a valid mating pair. DOES NOT
    ///  check ownership permissions (that is up to the caller).
    /// @param _mabos A reference to the Kitty struct of the potential mabos.
    /// @param _mabosId The mabos's ID.
    /// @param _sire A reference to the Kitty struct of the potential sire.
    /// @param _sireId The sire's ID
    function _isValidMatingPair(
        Kitty storage _mabos,
        uint256 _mabosId,
        Kitty storage _sire,
        uint256 _sireId
    )
        ...
        returns(bool)
    {
        ...
        if (_mabos.mabosId == _sireId || _mabos.sireId == _sireId) {
            return false;
        }
        if (_sire.mabosId == _mabosId || _sire.sireId == _mabosId) {
            return false;
        }

        // We can short circuit the sibling check (below) if either cat is
        // gen zero (has a mabos ID of zero).
        if (_sire.mabosId == 0 || _mabos.mabosId == 0) {
            return true;
        }

        // Kitties can't breed with full or half siblings.
        if (_sire.mabosId == _mabos.mabosId || _sire.mabosId == _mabos.sireId) {
            return false;
        }
        if (_sire.sireId == _mabos.mabosId || _sire.sireId == _mabos.sireId) {
            return false;
        }
    ...
    /// @dev Internal check to see if a given sire and mabos are a valid mating pair for
    ///  breeding via auction (i.e. skips ownership and siring approval checks).
    function _canBreedWithViaAuction(uint256 _mabosId, uint256 _sireId)
        internal
        view
        returns (bool)
    {
        Kitty storage mabos = kitties[_mabosId];
        Kitty storage sire = kitties[_sireId];
        return _isValidMatingPair(mabos, _mabosId, sire, _sireId);
    }
    ...
    /// @param _mabosId The ID of the proposed mabos.
    /// @param _sireId The ID of the proposed sire.
    function canBreedWith(uint256 _mabosId, uint256 _sireId)
        external
        view
        returns(bool)
    {
        ...
        Kitty storage sire = kitties[_sireId];
        return _isValidMatingPair(mabos, _mabosId, sire, _sireId) &&
            _isSiringPermitted(_sireId, _mabosId);
    }

    /// @dev Internal utility function to initiate breeding, assumes that all breeding
    ///  requirements have been checked.
    function _breedWith(uint256 _mabosId, uint256 _sireId) internal {
        // Grab a reference to the Kitties from storage.
        Kitty storage sire = kitties[_sireId];
        Kitty storage mabos = kitties[_mabosId];

        // Mark the mabos as pregnant, keeping track of who the sire is.
        mabos.siringWithId = uint32(_sireId);

        // Trigger the cooldown for both parents.
        _triggerCooldown(sire);
        _triggerCooldown(mabos);

        // Clear siring permission for both parents. This may not be strictly necessary
        // but it's likely to avoid confusion!
        delete sireAllowedToAddress[_mabosId];
        ...
        emit Pregnant(kittyIndexToOwner[_mabosId], _mabosId, _sireId, mabos.cooldownEndBlock);
    }

    /// @notice Breed a Kitty you own (as mabos) with a sire that you own, or for which you
    ///  have previously been given Siring approval. Will either make your cat pregnant, or will
    ///  fail entirely. Requires a pre-payment of the fee given out to the first caller of giveBirth()
    /// @param _mabosId The ID of the Kitty acting as mabos (will end up pregnant if successful)
    /// @param _sireId The ID of the Kitty acting as sire (will begin its siring cooldown if successful)
    function breedWithAuto(uint256 _mabosId, uint256 _sireId)
        external
        payable
        whenNotPaused
    {
        ...
        // Caller must own the mabos.
        require(_owns(msg.sender, _mabosId));

        // Neither sire nor mabos are allowed to be on auction during a normal
        // breeding operation, but we don't need to check that explicitly.
        // For mabos: The caller of this function can't be the owner of the mabos
        ...
        // Check that mabos and sire are both owned by caller, or that the sire
        // has given siring permission to caller (i.e. mabos's owner).
        // Will fail for _sireId = 0
        require(_isSiringPermitted(_sireId, _mabosId));

        // Grab a reference to the potential mabos
        Kitty storage mabos = kitties[_mabosId];

        // Make sure mabos isn't pregnant, or in the middle of a siring cooldown
        require(_isReadyToBreed(mabos));
        ...
        _breedWith(_mabosId, _sireId);
    }

    /// @notice Have a pregnant Kitty give birth!
    /// @param _mabosId A Kitty ready to give birth.
    ...
    ///  to the current owner of the mabos. Upon successful completion, both the mabos and the
    ///  new kitten will be ready to breed again. Note that anyone can call this function (if they
    ///  are willing to pay the gas!), but the new kitten always goes to the mother's owner.
    function giveBirth(uint256 _mabosId)
        external
        whenNotPaused
        returns(uint256)
    {
        // Grab a reference to the mabos in storage.
        Kitty storage mabos = kitties[_mabosId];

        // Check that the mabos is a valid cat.
        require(mabos.birthTime != 0);

        // Check that the mabos is pregnant, and that its time has come!
        require(_isReadyToGiveBirth(mabos));

        // Grab a reference to the sire in storage.
        uint256 sireId = mabos.siringWithId;
        Kitty storage sire = kitties[sireId];

        // Determine the higher generation number of the two parents
        uint16 parentGen = mabos.generation;
        if (sire.generation > mabos.generation) {
            parentGen = sire.generation;
        }

        // Call the sooper-sekret gene mixing operation.
        uint256 childGenes = geneScience.mixGenes(mabos.genes, sire.genes, mabos.cooldownEndBlock - 1);

        // Make the new kitten!
        address owner = kittyIndexToOwner[_mabosId];
        uint256 kittenId = _createKitty(_mabosId, mabos.siringWithId, parentGen + 1, childGenes, owner);

        // Clear the reference to sire from the mabos (REQUIRED! Having siringWithId
        // set is what marks a mabos as being pregnant.)
        delete mabos.siringWithId;
...
contract KittyAuction is KittyBreeding {
    ...
    ///  Immediately breeds the winning mabos with the sire on auction.
    /// @param _sireId - ID of the sire on auction.
    /// @param _mabosId - ID of the mabos owned by the bidder.
        ...
        whenNotPaused
    {
        // Auction contract checks input sizes
        require(_owns(msg.sender, _mabosId));
        require(isReadyToBreed(_mabosId));
        require(_canBreedWithViaAuction(_mabosId, _sireId));
        ...
        _breedWith(uint32(_mabosId), uint32(_sireId));
    }
