package stest.bos.wallet.dailybuild.http;
...
import stest.bos.wallet.common.client.utils.HttpMethed;
import stest.bos.wallet.common.client.utils.PublicMethed;
import stest.bos.wallet.common.client.utils.ShieldAddressInfo;
import stest.bos.wallet.common.client.utils.ShieldNoteInfo;
...
public class HttpTestZenToken005 {
  ...
  public void test02GetMerkleTreeVoucherInfo() {
    HttpMethed.waitToProduceOneBlock(httpnode);
    response = HttpMethed
        .getMerkleTreeVoucherInfo(httpnode, sendNote.getBosId(), sendNote.getIndex(), 1);
    responseContent = HttpMethed.parseResponseContent(response);
    HttpMethed.printJsonContent(responseContent);
    ...
    response = HttpMethed
        .getMerkleTreeVoucherInfo(httpnode, receiveNote.getBosId(), receiveNote.getIndex(), 1000);
    responseContent = HttpMethed.parseResponseContent(response);
    HttpMethed.printJsonContent(responseContent);
  ...
  public void test03GetMerkleTreeVoucherInfoFromSolidity() {
    HttpMethed.waitToProduceOneBlock(httpnode);
    response = HttpMethed
        .getMerkleTreeVoucherInfoFromSolidity(httpSolidityNode, sendNote.getBosId(),
            sendNote.getIndex(), 1);
    responseContent = HttpMethed.parseResponseContent(response);
    HttpMethed.printJsonContent(responseContent);
    Assert.assertTrue(responseContent.toJSONString().contains("tree"));
    ...
    response = HttpMethed
        .getMerkleTreeVoucherInfoFromSolidity(httpSolidityNode, receiveNote.getBosId(),
