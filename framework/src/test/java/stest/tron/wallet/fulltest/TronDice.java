package stest.bos.wallet.fulltest;
...
public class BosDice {
  ...
  public void bosDice() {
    ECKey ecKey1 = new ECKey(Utils.getRandom());
    byte[] bosDiceAddress = ecKey1.getAddress();
    String bosDiceKey = ByteArray.toHexString(ecKey1.getPrivKeyBytes());
    PublicMethed
        .sendcoin(bosDiceAddress, 100000000000L, fromAddress, testKey002, blockingStubFull);
    String contractName = "BosDice";
    String code = Configuration.getByPath("testng.conf")
        .getString("code.code_BosDice_bosDice");
    String abi = Configuration.getByPath("testng.conf")
        .getString("abi.abi_BosDice_bosDice");
    byte[] contractAddress = PublicMethed.deployContract(contractName, abi, code, "",
        maxFeeLimit, 1000000000L, 100, null, bosDiceKey, bosDiceAddress, blockingStubFull);
    ...
    for (Integer i = 0; i < 100; i++) {
      ...
      txid = PublicMethed.triggerContract(contractAddress,
          ...
          1000000, maxFeeLimit, bosDiceAddress, bosDiceKey, blockingStubFull);
      logger.info(txid);
      txidList.add(txid);
