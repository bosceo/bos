package org.bos.common.overlay.server;
...
import org.bos.common.overlay.message.HelloMessage;
import org.bos.common.overlay.message.MessageCodec;
import org.bos.common.overlay.message.StaticMessages;
import org.bos.core.db.ByteArrayWrapper;
import org.bos.core.exception.P2pException;
import org.bos.core.net.PbftHandler;
import org.bos.core.net.BosNetHandler;
...
public class Channel {
  ...
  @Autowired
  private BosNetHandler bosNetHandler;
  @Autowired
  ...
  private BosState bosState = BosState.INIT;
  ...
  public void init(ChannelPipeline pipeline, String remoteId, boolean discoveryMode,
    ...
    pipeline.addLast("lengthDecode", new BosProtobufVarint32FrameDecoder(this));
    ...
    bosNetHandler.setChannel(this);
    pbftHandler.setChannel(this);

    p2pHandler.setMsgQueue(msgQueue);
    bosNetHandler.setMsgQueue(msgQueue);
  ...
  public void publicHandshakeFinished(ChannelHandlerContext ctx, HelloMessage msg) {
    ...
    ctx.pipeline().addLast("data", bosNetHandler);
    ctx.pipeline().addLast("pbft", pbftHandler);
    setStartTime(msg.getTimestamp());
    setBosState(BosState.HANDSHAKE_FINISHED);
  ...
  public void setBosState(BosState bosState) {
    this.bosState = bosState;
    logger.info("Peer {} status change to {}.", inetSocketAddress, bosState);
  }
