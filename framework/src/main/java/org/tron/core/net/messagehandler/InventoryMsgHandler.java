...
@Slf4j(topic = "net")
@Component
public class InventoryMsgHandler implements BosMsgHandler {
  ...
  @Override
  public void processMessage(PeerConnection peer, BosMessage msg) {
  ...
  private boolean check(PeerConnection peer, InventoryMessage inventoryMessage) {
    ...
    if (type.equals(InventoryType.BOS)) {
      int count = peer.getNodeStatistics().messageStatistics.bosInBosInventoryElement.getCount(10);
