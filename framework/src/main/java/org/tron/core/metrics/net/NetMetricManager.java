package org.bos.core.metrics.net;
...
public class NetMetricManager {
  ...
  private void setNetInfo(NetInfo netInfo) {
    //set connection info
    netInfo.setConnectionCount(bosNetDelegate.getActivePeer().size());
    int validConnectionCount = 0;
    for (PeerConnection peerConnection : bosNetDelegate.getActivePeer()) {
