...
Having a tailored actuator is key to building a java-bos-based customized public chain. This article illustrates how to develop a java-bos-based `SumActuator`.
...
Currently, contracts supported by java-bos are defined under `src/main/protos/core/contract` directory in protocol module. First creating a `math_contract.proto` file under this directory and declaring `SumContract`. You can also implement any mathematical calculation you want, such as `Subtraction`.
...
syntax = "proto3";
package protocol;
option java_package = "org.bos.protos.contract"; //Specify the name of the package that generated the Java file
option go_package = "github.com/bosprotocol/grpc-gateway/core";
message SumContract {
    int64 param1 = 1;
    int64 param2 = 2;
...
Meanwhile, register the new contract type in `Transaction.Contract.ContractType` emuneration within the `src/main/protos/core/Bos.proto` file. Important data structures, such as transactions, accounts and blocks, are defined in the `Bos.proto` file:
...
```
At last, recompile the modified proto files. Compiling the java-bos project directly will compile the proto files as well, `protoc` command is also supported.

*Currently, java-bos uses protoc v3.4.0. Please keep the same version when compiling by `protoc` command.*
...
# or build via protoc
protoc -I=src/main/protos -I=src/main/protos/core --java_out=src/main/java  Bos.proto
...
For now, the default Actuator supported by java-bos is located in `org.bos.core.actuator`. Creating `SumActuator` under this directory:
...
INFO [net] BosNetService start successfully.
