...
public class StorageMarket {
  ...
  private long exchange_to_supply(boolean isBOS, long quant) {
    logger.info("isBOS: " + isBOS);
    long balance = isBOS ? dynamicPropertiesStore.getTotalStoragePool() :
...
//    }
  ...
  private long exchange_to_supply2(boolean isBOS, long quant) {
    logger.info("isBOS: " + isBOS);
    long balance = isBOS ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    logger.info("balance: " + balance);
...
//    }
  ...
  private long exchange_from_supply(boolean isBOS, long supplyQuant) {
    long balance = isBOS ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    supply -= supplyQuant;
  ...
  public long exchange(long from, boolean isBOS) {
    long relay = exchange_to_supply(isBOS, from);
    return exchange_from_supply(!isBOS, relay);
  }
