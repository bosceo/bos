...
# Function: to start, stop and restart java-bos.
...
JAR_FILE_PATH="./build/libs/java-bos.jar"
PID_FILE_PATH="java-bos.pid"
LOG_FILE_PATH="java-bos.log"
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start java-bos."
        else
            echo "Failed to start java-bos."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop java-bos."
        else
            echo "Failed to stop java-bos."
        fi
    ;;
    restart)
