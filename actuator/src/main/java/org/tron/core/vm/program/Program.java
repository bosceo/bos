...
package org.bos.core.vm.program;
...
import static org.bos.common.utils.ByteUtil.stripLeadingZeroes;
...
import org.bos.common.utils.BIUtil;
import org.bos.common.utils.ByteUtil;
import org.bos.common.utils.FastByteComparisons;
import org.bos.common.crypto.Hash;
...
import org.bos.core.exception.BosException;
import org.bos.core.utils.TransactionUtil;
import org.bos.core.vm.EnergyCost;
import org.bos.core.vm.MessageCall;
import org.bos.core.vm.OpCode;
import org.bos.core.vm.PrecompiledContracts;
import org.bos.core.vm.VM;
import org.bos.core.vm.VMConstant;
import org.bos.core.vm.VMUtils;
import org.bos.core.vm.config.VMConfig;
import org.bos.core.vm.program.invoke.ProgramInvoke;
import org.bos.core.vm.program.invoke.ProgramInvokeFactory;
import org.bos.core.vm.program.invoke.ProgramInvokeFactoryImpl;
import org.bos.core.vm.program.listener.CompositeProgramListener;
import org.bos.core.vm.program.listener.ProgramListenerAware;
import org.bos.core.vm.program.listener.ProgramStorageChangeListener;
import org.bos.core.vm.repository.Repository;
import org.bos.core.vm.trace.ProgramTrace;
import org.bos.core.vm.trace.ProgramTraceListener;
...
import org.bos.protos.contract.SmartContractOuterClass.SmartContract.Builder;
...
public class Program {
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = TransactionTrace.convertToBosAddress(getContractAddress().getLast20Bytes());
    byte[] obtainer = TransactionTrace.convertToBosAddress(obtainerAddress.getLast20Bytes());
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
      boolean isCreate2) {
    byte[] senderAddress = TransactionTrace.convertToBosAddress(this.getContractAddress().getLast20Bytes());
    ...
    if (VMConfig.allowTvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setBosHash(ByteString.copyFrom(rootTransactionId));
        }
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(TransactionTrace.convertToBosAddress(getContractAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = TransactionTrace.convertToBosAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = TransactionTrace.convertToBosAddress(getContractAddress().getLast20Bytes());
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(TransactionTrace.convertToBosAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit().getCode(TransactionTrace.convertToBosAddress(address.getLast20Bytes()));
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] bosAddr = TransactionTrace.convertToBosAddress(address.getLast20Bytes());
    AccountCapsule account = getContractState().getAccount(bosAddr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(bosAddr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(bosAddr, contract);
        }
      } else {
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState()
        .getBalance(TransactionTrace.convertToBosAddress(address.getLast20Bytes()));
    return new DataWord(balance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState()
        .getContract(TransactionTrace.convertToBosAddress(address.getLast20Bytes()));
  ...
  public DataWord storageLoad(DataWord key) {
    DataWord ret = getContractState()
        .getStorageValue(TransactionTrace.convertToBosAddress(getContractAddress().getLast20Bytes()),
            key.clone());
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState()
        .getTokenBalance(TransactionTrace.convertToBosAddress(address.getLast20Bytes()),
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    byte[] senderAddress = TransactionTrace.convertToBosAddress(this.getCallerAddress().getLast20Bytes());
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(TransactionTrace.convertToBosAddress(msg.getType().callIsDelegate()
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(BosException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(BosException e) {
      return new PrecompiledContractException(e.getMessage());
    }
