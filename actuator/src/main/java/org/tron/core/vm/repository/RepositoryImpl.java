...
import org.bos.core.config.Parameter;
...
public class RepositoryImpl implements Repository {
  ...
  public void putStorageValue(byte[] address, DataWord key, DataWord value) {
    address = TransactionTrace.convertToBosAddress(address);
  ...
  public Storage getStorage(byte[] address) {
    ...
    ContractCapsule contract = getContract(address);
    if (contract != null && !ByteUtil.isNullOrZeroArray(contract.getBosHash())) {
      storage.generateAddrHash(contract.getBosHash());
    }
    return storage;
  }
